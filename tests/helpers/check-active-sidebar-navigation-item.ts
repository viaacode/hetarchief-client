...
export async function checkActiveSidebarNavigationItem(
	...
	const sidebarSelector = `${moduleClassSelector('SidebarLayout', 'l-sidebar__navigation')} >> nth=${navigationListIndex}`;
	const activeSelector = `${sidebarSelector} >> ${moduleClassSelector('ListNavigation', 'c-list-navigation__item--active')}`;
