// noinspection DuplicatedCode,BadExpressionStatementJS
...
const ObjectDetailPage: NextPage<ObjectDetailPageProps> = ({ title, description, url }) => {
	...
	// TODO set active tab
	const activeTab: ObjectDetailTabs = ObjectDetailTabs.Media;
	...
	const iiifViewerId: string | undefined = mediaInfo?.schemaIdentifier;
	const [miradorInstance, setMiradorInstance] = useState<MiradorInstance | null>(null);
	const [miradorLib, setMiradorLib] = useState<MiradorLib | null>(null);
	...
	const initIiifViewer = useCallback(async () => {
		if (iiifViewerId && isBrowser()) {
			const mirador = (await import('mirador')).default as MiradorLib;
			setMiradorLib(mirador);
			const miradorInstanceTemp = mirador.viewer(
				{
					...miradorConfig,
					id: iiifViewerId,
				},
			...
			(window as any).miradorInstance = miradorInstanceTemp;
			setMiradorInstance(miradorInstanceTemp);
		}
	}, [iiifViewerId]);
	...
	useEffect(() => {
		...
		if (isMobile && (activeTab as any) === ObjectDetailTabs.Metadata) {
	...
	const iiifZoom = (multiplier: number): void => {
		...
		const viewer = Object.values(state?.viewers || {})[0];
		miradorInstance?.store?.dispatch(
			miradorLib?.actions.updateViewport(Object.keys(state?.windows || {})[0], {
				x: viewer.x,
				y: viewer.y,
				zoom: viewer.zoom * multiplier,
				flip: false,
				rotation: 0,
			})
		);
			...
			<article
				className={clsx('p-object-detail__wrapper', {
					...
					'p-object-detail__wrapper--metadata':
						(activeTab as any) === ObjectDetailTabs.Metadata,
