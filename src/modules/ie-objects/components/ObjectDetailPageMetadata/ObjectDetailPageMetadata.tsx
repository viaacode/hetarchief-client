			...
			switch (format) {
				case MetadataExportFormats.fullNewspaperZip:
					...
					setOnConfirmCopyright(() => () => {
						window.open(
							`${publicRuntimeConfig.PROXY_URL}/${NEWSPAPERS_SERVICE_BASE_URL}/${mediaInfo.schemaIdentifier}/${IE_OBJECTS_SERVICE_EXPORT}/zip`
						);
				...
				case MetadataExportFormats.onePageNewspaperZip:
					...
					setOnConfirmCopyright(() => () => {
						window.open(
							`${publicRuntimeConfig.PROXY_URL}/${NEWSPAPERS_SERVICE_BASE_URL}/${mediaInfo.schemaIdentifier}/${IE_OBJECTS_SERVICE_EXPORT}/zip?page=${currentPageIndex}`
						);
						handleOnDownloadEvent();
					});
					break;
		...
		return (
			<>
				...
				<MetadataList allowTwoColumns={true}>
					...
					{renderSimpleMetadataField(
						tText('modules/ie-objects/const/index___stuk'),
						renderIsPartOfValue(mediaInfo.isPartOf, IsPartOfKey.stuk)
					)}
					{renderSimpleMetadataField(
					...
					{renderSimpleMetadataField(
						tText('modules/ie-objects/const/index___registratie'),
						renderIsPartOfValue(mediaInfo.isPartOf, IsPartOfKey.registratie)
					)}
					{renderSimpleMetadataField(
						tText('modules/ie-objects/const/index___serienummer'),
						renderIsPartOfValue(mediaInfo.isPartOf, IsPartOfKey.serienummer)
					)}
					{renderSimpleMetadataField(
						tText('modules/ie-objects/const/index___seizoennummer'),
						renderIsPartOfValue(mediaInfo.isPartOf, IsPartOfKey.seizoennummer)
					)}
					{renderSimpleMetadataField(
					...
					{renderSimpleMetadataField(
						...
						mediaInfo?.maintainerIiifAgreement && activeFile?.storedAt && (
